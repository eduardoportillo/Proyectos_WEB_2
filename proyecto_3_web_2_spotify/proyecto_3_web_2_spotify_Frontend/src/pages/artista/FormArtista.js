...
const FormArtista = props => {
	...
	const insertarPersona = dataArtista => {
		axios
			.post(`${apiRootPath}artistas`, dataArtista, {})
			.then(response => {
	...
	const actualizarPersona = dataArtista => {
		axios
			.patch(`${apiRootPath}artistas/${id}`, dataArtista, {})
			.then(response => {
				console.log('recibido', response.data);
				history.push('/artistas');
			})
			...
			<Col md={{span: 6, offset: 3}}>
				<Card className='mt-3'>
					<Card.Body>
						...
						{/* // [ ] falta completar Genero musical  */}
						<div>
							<select
							...
							>
								{listaGenero.map(item => (
									<option value={item.id != '' ? item.id : 1}>
										{item.nombre_genero}
									</option>
								))}
							</select>
