...
class Lista extends Component {
    ...
    getContent() {
        var data = Parent.Actions.iniciado();
    ...
    render() {
        return (
            <SPage title={'Lista de ' + Parent.component} disableScroll center preventBack>
                {this.getContent()}
                {!user.Actions.validateSession(null, true) ? (<SButtom type={"float"} onPress={() => {
                    SNavigation.navigate("login")
                }}>
                    <SIcon name={"Salir"} height={50} />
                </SButtom>
                ) : null}
            </SPage>
        );
